...
const BodyPart = ({item, icon, bodyPart, setBodyPart}) => {
  ...
  return (
    ...
    <img className="bodyIcon" src= {`/icons/bodyParts/${item.replace(/\s/g, '')}.png`} alt="body part image" />
    <Typography style = {{ fontWeight: 'bold', color: 'rgb(244,70,48)'}} >{item.toUpperCase()}</Typography>
    </Stack>
